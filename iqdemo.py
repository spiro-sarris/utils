...
fftsize = 1024					# number of FFT spectrum bins
ff_kHz = (np.linspace(0,fs,fftsize)-fs/2)/1e3
...
SIG_C = np.fft.fft(sig_c, fftsize)
SIG_R = np.fft.fft(sig_r, fftsize)
SIG_H = np.fft.fft(sig_h, fftsize)
...
plt.plot(ff_kHz, mylib.todB(np.abs(SIG_C_SHIFT)**2))
...
plt.plot(ff_kHz, mylib.todB(np.abs(SIG_R_SHIFT)**2))
plt.xlabel('frequency(kHz)')
plt.xlim((ff_kHz[0],ff_kHz[-1]))
plt.grid(True)
...
plt.plot(ff_kHz, mylib.todB(np.abs(SIG_H_SHIFT)**2))
plt.xlabel('frequency(kHz)')
plt.xlim((ff_kHz[0],ff_kHz[-1]))
plt.grid(True)
